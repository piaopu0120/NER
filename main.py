...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    data.build_alphabet(train_file)  # train/dev/test都加入了词典构造 不是造了三个，是造了一个词典，加入了三个数据集的信息
    data.build_alphabet(dev_file)
    data.build_alphabet(test_file)
    data.build_gaz_file(gaz_file) # 建立了word embedding的词典
    data.build_gaz_alphabet(train_file,count=True) # 如果BMES的词的重新组合是gaz里有的，则把实体加入到gaz_alphabet里，统计词频
...
if __name__ == '__main__':
    ...
    gaz_file = "./data/ctb.50d.vec" # gaz_file->gazetters embedding词典，里面是字+数字向量
    ...
    if status == 'train':
        if os.path.exists(save_data_name):
            ...
            data.build_word_pretrain_emb(char_emb) # 建立词语到embedding的映射
            data.build_biword_pretrain_emb(bichar_emb)
            data.build_gaz_pretrain_emb(gaz_file)

            print('Dumping data')
            with open(save_data_name, 'wb') as f:
                pickle.dump(data, f) # 序列化对象
            set_seed(seed_num)
